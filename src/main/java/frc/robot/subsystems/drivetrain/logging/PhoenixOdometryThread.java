...
public class PhoenixOdometryThread extends Thread {
    ...
    public Queue<Double> registerSignal(StatusSignal<Angle> signal) {
        ...
        signalsLock.lock();
        CommandSwerveDrivetrainLogging.odometryReadLock.lock();
        try {
            ...
            signalsLock.unlock();
            CommandSwerveDrivetrainLogging.odometryReadLock.unlock();
        }
        return queue;
    }
            ...
            try {
                if (isCANFD && phoenixSignals.length > 0) {
                    BaseStatusSignal.waitForAll(
                            2.0 / TunerConstants.ODOMETRY_FREQUENCY, phoenixSignals);
                } else {
