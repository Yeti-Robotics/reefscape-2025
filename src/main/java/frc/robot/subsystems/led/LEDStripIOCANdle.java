...
public class LEDStripIOCANdle implements LEDStripIO {
    ...
    private final LarsonAnimation idleRedAnimation =
            new LarsonAnimation(255, 0, 0, 0, 0.03, numLed, LarsonAnimation.BounceMode.Center, 2);

    // Yeti blue fr fr
    private final LarsonAnimation idleBlueAnimation =
            new LarsonAnimation(
                    84, 182, 229, 0, 0.03, numLed, LarsonAnimation.BounceMode.Center, 2);

    private final StrobeAnimation coralStowedAnimation =
            new StrobeAnimation(255, 255, 255, 0, 0.03, numLed);

    private final LarsonAnimation coralIntakeAnimation =
            new LarsonAnimation(
                    255, 255, 255, 0, 0.03, numLed, LarsonAnimation.BounceMode.Center, 5);

    private final LarsonAnimation algaeIntakeAnimation =
            new LarsonAnimation(
                    79, 181, 165, 0, 0.03, numLed, LarsonAnimation.BounceMode.Center, 5);

    private final FireAnimation flameTune = new FireAnimation(0.8, 0.75, numLed, 1, 0.3);

    // nickelodeon
    private final StrobeAnimation nick = new StrobeAnimation(240, 119, 99, 0, 0.03, numLed);
