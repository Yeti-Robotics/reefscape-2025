...
public class PhotonAprilTagSystem extends SubsystemBase implements AprilTagSubsystem {
        ...
        for (PhotonPipelineResult result : results) {
            ...
            if (result.hasTargets()) {
                ...
                if (result.getTimestampSeconds() > currentBestDetectionTimestamp
                        && isFiducialOfInterest(bestDetection.getFiducialId())) {
                ...
                for (PhotonTrackedTarget target : result.getTargets()) {
                    if (target.getFiducialId() != -1
                            && isFiducialOfInterest(target.getFiducialId())) {
    ...
    private boolean isFiducialOfInterest(int id) {
        return (fiducialIds == null || fiducialIds.length < 1)
                || Arrays.stream(fiducialIds).anyMatch(e -> e == id);
    }
